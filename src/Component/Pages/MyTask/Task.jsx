...
const Task = ({task}) => {
  const {taskName,description,completed} = task
  ...
  return (
    <div className="card w-96 bg-base-100 shadow-xl mx-auto my-5">
      ...
      <div className="card-body justify-start items-start">
        {completed ? <s className="card-title">{taskName}</s>:<h2 className="card-title">{taskName}</h2>}
        {completed ?<s>Description: {description}</s>:<p>Description: {description}</p>}
      </div>
